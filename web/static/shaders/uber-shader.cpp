...
vec3 gamma(vec3 val) {
  return pow(val, vec3(1.0/GAMMA_VALUE));
}
...
varying vec4 vPosition_WS;
  ...
  uniform vec3 ambientLightColor;
    ...
    vec3 FuseSpecular(float specPower, float NdotH, float HdotL, float NdotL) {
      float FG = 0.25 / (pow(NdotH, 3.0) + 1.0/32.0);
      float D = 0.5 * (specPower + 1.0) * pow(NdotH, specPower);
      float DFG = D * FG;
  ...
  vec2 uvOffset = vec2(0.0, 0.0);
    ...
    float NdotV = dot( eyeVector_VS, normal_VS );
      ...
      float mipBias = 0.0;
        ...
        mipBias = (1.0 - glossValue) * 5.0;
      #endif
      vec3 cameraToVertex = normalize( vPosition_WS.xyz - cameraPosition );
      vec3 vReflectWorldSpace = (vec4( reflect( cameraToVertex, (vec4(normal_VS, 0.0) * viewMatrix).xyz ), 0.0 )).xyz;
        ...
        vec4 reflectedColor = texture2D( environmentTexture2D, vec2(-1.0, 1.0) * (gl_FragCoord.xy - screenDimensions.xy) / screenDimensions.zw, mipBias );

      #elif ( REFLECTIONS == 3 )
        vec2 sampleUV;
        sampleUV.y = clamp( vReflectWorldSpace.y * -0.5 + 0.5, 0.0, 1.0);

        sampleUV.x = atan( vReflectWorldSpace.z, vReflectWorldSpace.x ) * 0.15915494309189533576888376337251 + 0.5;
    ...
    vec3 totalOther = vec3( 0.0 );
            ...
            if ( frustumTest ) {
              ...
              shadowCoord.z += shadowBias[ s ];
                ...
                shadow = dot(shadowValueVector, vec4(1.0));
                  ...
                  float totalDepth = dot(depthValues, vec4(1.0));
                  ...
                  shadowValuesScatter[ s ] = exp( (1.0 - scatterScale) * -exponent );
                #endif
          ...
          for ( int p = 0; p < MAX_POINT_LIGHTS; p ++ ) {
            ...
            vec3 pointVector_VS = normalize((viewMatrix * vec4( pointVector_WS, 0.0 )).xyz);
          ...
          for ( int i = 0; i < MAX_DIR_LIGHTS; i ++ ) {
            ...
            vec3 lightDirection_VS = (viewMatrix * vec4( directionalLightDirection[ i ], 0.0 )).xyz;
    ...
    if ( renderModeNormals == 1 ) {
      normal_VS.xyz =  0.5 * normal_VS.xyz + 0.5;
      gl_FragColor = vec4( normal_VS.xyz, 1.0  );
    }
    else {
